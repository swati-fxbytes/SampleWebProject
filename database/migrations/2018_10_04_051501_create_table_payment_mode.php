...
class CreateTablePaymentMode extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        if (!Schema::hasTable('payment_mode')) {
            Schema::create('payment_mode', function (Blueprint $table) {
                $table->increments('payment_mode_id')->unsigned()->comment('Payment Mode ID');
                $table->integer('user_id')->unsigned()->index()->comment('Foreign key from users table');
                $table->string('payment_mode',255)->nullable()->comment('Name of the payment mode');

...
class CreateTablePatientLaboratoryTest extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
         if (!Schema::hasTable('patient_laboratory_test')) {
            Schema::create('patient_laboratory_test', function (Blueprint $table) {
                $table->increments('plt_id')->unsigned()->comment('Patients Laboratory Test unique id');
                ...
                $table->integer('plt_type')->comment('domestic_factor_id from static config data');
                $table->integer('plt_type_id')->comment('domestic_factor_id from static config data');
                $table->string('plt_value',255)->nullable()->comment('domestic_factor_value ');
    ...
    {
         Schema::dropIfExists('patient_laboratory_test');
    }
}

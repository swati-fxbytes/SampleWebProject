...
class CreateTableHistoryOfPatientIllness extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
         if (!Schema::hasTable('history_of_patient_illness')) {
            Schema::create('history_of_patient_illness', function (Blueprint $table) {
                $table->increments('hopi_id')->unsigned()->comment('History of patient illness unique id');
                ...
                $table->integer('hopi_type')->comment('hopi_type from static config data');
                $table->integer('hopi_type_id')->comment('hopi_type_id from static config data');
                $table->string('hopi_value',255)->nullable()->comment('hopi_value ');
    ...
    {
         Schema::dropIfExists('history_of_patient_illness');
    }
}

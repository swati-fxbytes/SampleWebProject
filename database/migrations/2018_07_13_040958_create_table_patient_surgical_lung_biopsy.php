...
class CreateTablePatientSurgicalLungBiopsy extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
       if (!Schema::hasTable('patient_surgical_lung_biopsy')) {
            Schema::create('patient_surgical_lung_biopsy', function (Blueprint $table) {
                $table->increments('pslb_id')->comment('Primary key patient_surgical_lung_biopsy table');
                ...
                $table->date('pslb_date')->nullable()->comment('Date of  surgical lung biopsy');
                $table->tinyInteger('pslb_is_happen')->unsigned()->nullable()->comment('1 for yes, 2 for no');
    ...
    {
        Schema::dropIfExists('patient_surgical_lung_biopsy');
    }
}

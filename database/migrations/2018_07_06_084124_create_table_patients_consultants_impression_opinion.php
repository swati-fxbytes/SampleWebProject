...
class CreateTablePatientsConsultantsImpressionOpinion extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        if (!Schema::hasTable('patient_consultants_impression_opinion')) {
            Schema::create('patient_consultants_impression_opinion', function (Blueprint $table) {
                $table->increments('pcio_id')->unsigned()->comment('Patients Consultants Impression Opinion unique id');
                ...
                $table->integer('pcio_type_id')->comment('consultants list unique from static config data');
                $table->string('pcio_value',255)->nullable()->comment('pcio_value ');
    ...
    {
        Schema::dropIfExists('patient_consultants_impression_opinion');
    }
}

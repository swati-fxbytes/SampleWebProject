...
class vitals extends Model {
    ...
    */
    ...
    protected $tableDoctorsSpecialisations = 'doctors_specialisations';
    ...
    */
    public function getPrimarySpecialisation($userId){
        $result = DB::table($this->tableDoctorsSpecialisations)
                        ->select("spl_id")
    ...
    */
     ...
     * @return                object Array of medical history records
     */
    public function getV1PatientVitalsByFactorIdPatientIdAndDoctorId($patId, $doctorId, $extra = [])
    {
        $dateType   = Config::get('constants.USER_VIEW_DATE_FORMAT_CARBON');
        $dbDateType = Config::get('constants.DB_SAVE_DATE_FORMAT');
        $fectorId   = $extra['fector_id'];
        $name       = $extra['name'];
        $queryResult = DB::table($this->table)
                            ->select(DB::raw($this->table . ".created_at::date, " . $this->table . ".created_at, DATE(" . $this->table . ".created_at) as date, fector_value as datavalue, " . $this->table . ".visit_id"))
                            ->join($this->tablePatientVisit, function ($join) {
                                $join->on($this->tablePatientVisit . '.visit_id', '=', $this->table . '.visit_id')
                                ->where($this->tablePatientVisit . '.is_deleted', '=', Config::get('constants.IS_DELETED_NO'), 'and');
                            })
     ...
     * @return                object Array of medical history records
     */
    public function getPatientVitalsByPatientIdAndDoctorIdAndVisitId($patId, $doctorId, $visitId)
    {
        $res =  DB::select(
                    DB::raw( "
                        SELECT 
